...
import placeholderRecordImg from '~/assests/placeholder-record.png'
...
export default function Collection() {
  ...
  const handleCurrencyChange = async (newCurrency: string) => {
    ...
    const handleSearch = (query: string) => {
      if (!query) {
        setFilteredRecords(collection);
        return;
      }
  
      const lowerCaseQuery = query.toLowerCase();
      const filtered = collection.filter((record: any) => (
        record.basic_information.artists[0].name.toLowerCase().includes(lowerCaseQuery) ||
        record.basic_information.title.toLowerCase().includes(lowerCaseQuery) ||
        record.basic_information.year.toString().includes(lowerCaseQuery) ||
        record.basic_information.genres.join(", ").toLowerCase().includes(lowerCaseQuery)
      ));
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 lg:grid-cols-4 py-8 mx-auto max-w-screen-xl lg:py-16 justify-items-center">
          {filteredRecords.map((record: any) => (
            <div key={record.id} className="max-w-xs bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
              <img className=" rounded-t-lg max-h-80" src={record.basic_information.cover_image || placeholderRecordImg} alt={record.basic_information.title} />
              <div className="p-5">
